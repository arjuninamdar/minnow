...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring )
{
	...
	if (data == "") {
		...
		if (first_index < start_index || first_index >= start_index + available_capacity) 
			return;
	} else if ((first_index + data.size() <= start_index) || (first_index >= start_index + available_capacity)) {
		return;
	}

	// if the substring is out of bounds to the right, we want to ensure that we discard irrelevant bytes
	if (static_cast<int64_t>(first_index + data.size()) - static_cast<int64_t>(start_index + available_capacity) > 0) {
		data = data.substr(0, data.length() - (first_index + data.size()) + (start_index + available_capacity));
	}

	// if the substring is before the start index, discard the beginning up to the start_index (we've already seen these bytes)
	...
	fill(init.begin() + (first_index - start_index), init.begin() + (first_index - start_index) + data.length(), true);
