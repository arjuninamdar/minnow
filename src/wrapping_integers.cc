...
uint64_t Wrap32::unwrap( Wrap32 zero_point, uint64_t checkpoint ) const
{
	...
	for (uint64_t possible_value : possible_values) {
		uint64_t new_distance = (possible_value > checkpoint) ? possible_value - checkpoint : checkpoint - possible_value;
		uint64_t old_distance = (closest_value > checkpoint) ? closest_value - checkpoint : checkpoint - closest_value;
